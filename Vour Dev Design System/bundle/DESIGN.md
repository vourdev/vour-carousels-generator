# Vour Dev — DESIGN.md  ·  Update 6 (self-contained)

> **THIS FILE IS THE SINGLE SOURCE OF TRUTH.** Any consumer file (skill manifest, README) is a thin pointer to this document. **If a rule is stated in the consumer's skill file AND in DESIGN.md, DESIGN.md wins.** The brand mark is embedded as a base64 data URL in §3a — you do not need any other file, image, or asset to use this system. Pair it with `MAKING_CAROUSELS.md` for the build procedure.
>
> **What changed in Update 6.**
>
> 1. **§16 · Slide-introduction contract.** Every slide's intro block (counter → eyebrow → headline → description) has HARD word / char / line caps per size. Eyebrows cap at 3 words / 20 chars; descriptions cap at 100 chars on mockup slides and 140 chars on text-only slides. Includes the "conjunction check" heuristic (more than one `dan / atau / tapi / kalau / karena` = the sentence is too long).
> 2. **§17 · Single-mockup fills the full width.** ONE mockup → spans `.diag-wrap` (100% wide). TWO → 1fr / 1fr grid (`ImagePlatePair`, `.diag-bars`, `SplitPanel`). THREE–FOUR → `MediaGrid` 2×2. Never a single mockup in a half column with empty space beside it.
> 3. **§18 · Extended mockup catalog.** Five new mockup roles for topics that don't fit the §13 diagram vocabulary: full-bleed `ImagePlate`, before/after `ImagePlatePair`, `MediaGrid` (2×2 gallery), `BigStat` (metric), `PullQuote` (testimonial), `SplitPanel` (text + image side-by-side).
> 4. **Skill manifest is now a stub.** All rules moved into this file. Consumer `SKILL.md` only carries skill metadata + the workflow contract ("read `DESIGN.md` first"). This kills the long-running "skill and design.md conflict" bug.
>
> **What still holds from earlier updates.** §14 (mockup proportion contract), §15 (`vourdev-meta` automation block), Update 4 (editorial surface only), Update 3 (§13 diagram vocabulary), Update 2 (opaque icon containers) are unchanged.
>
...
13. **Mockup slides pass the §13 proportion contract.** Title, eyebrow, description, and mock-screen NEVER overlap. Headline uses `.compact` (88px). Description ≤ 2 lines / 120 chars. `.diag-wrap` has `flex:1; min-height:0`.
14. **The `<script id="vourdev-meta">` block is present in `<head>`,** with valid JSON (`title`, `caption`, `hashtags`). Never omit; the export pipeline depends on it.
...
## 13 · Mockup slide proportion contract  ★ Update 5

> Slides carrying a **mock-screen** — Terminal, Illustrated Scene, Permission Table, Comparison Bars, or any diagram role — must follow this exact vertical stack. If any element overlaps another, cut copy. **Never edit the CSS.**

### 13.1 The locked stack
...
2. Mockup-slide headline uses `.compact` (88px). Reserving 104 / 128 for cover and text-only slides guarantees the diagram gets ≥ 500px of vertical room.
3. Mockup-slide description caps at 2 lines / ~120 chars. Cut copy — never shrink font, never remove the 40px gap.
4. Terminal body caps at 8 code lines, each ≤ 44 mono chars.
...
### 13.3 The pre-flight visual check
...
### 13.4 Which mockup role to pick
...
### 13.5 Terminal syntax palette (pinned)

Terminal is the go-to mockup for database-query, config, and skill-file topics. Background `#1F0904` (never pure black). Colours locked:
...
## 14 · Automation metadata block  ★ Update 5
...
*Update 5 · 2026-07 · tightens mockup-slide proportion contract (§13) and requires the `vourdev-meta` automation block (§14). Everything else from Update 4 stands unchanged.*
...
| `PullQuote` | fills wrap | 100% | Quote is Sora 700 · 56 px, author is JetBrains Mono 500 · 24 px orange. |
...
4. `PullQuote` opens with one large orange quote-mark glyph; the quote body is `--ed-ink`, never orange. One accent span in the quote body is allowed (`<span class="a">`), same rule as headline.
...
*Update 6 · 2026-07 · adds §16 slide-introduction contract, §17 single-mockup full-width rule, §18 extended mockup catalog (BigStat, PullQuote, SplitPanel, MediaGrid, plus `ImagePlate` full-bleed / pair patterns). Consumer `SKILL.md` is now a thin pointer to this file. Update 5 (§13, §14), Update 4 (editorial surface), Update 3 (diagram vocabulary §12), Update 2 (opaque icon containers) all continue unchanged.*
