...
| Where | What |
|---|---|
| `bundle/TEMPLATE-editorial-v3.html` | Every pattern as a copy-paste `<section>`. |
...
| `ui_kits/ai-guide-non-tech/index.html` | Full 8-slide working carousel wiring the whole vocabulary together. |
...
Terminal is the default for **database queries** and any code snippet. Background `#1F0904`, syntax classes `.cmt` `.key` `.val` `.kw` `.num` (pinned hex in §13.5-equivalent — see `bundle/DESIGN.md §13.5`).
...
| `bundle/TEMPLATE-editorial-v3.html` | (Continues to be the single starting-template. Copy the section from the relevant slide sample into the template; do not fork the template per role.) |
