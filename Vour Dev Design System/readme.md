...
5. **Bundle template updated.** `bundle/TEMPLATE-editorial-v3.html` ships with the metadata placeholder in `<head>`, adds `.diag-wrap` + `h1.compact` classes, and every mockup slide already uses the locked stack.
...
3. **Slide 14 `mock-screen` pattern redesigned.** Title / eyebrow / description / mock-screen now sit proportionally; the locked panel uses a dark `.mock-lock` pill (lucide icon + label) so the overlay never overlaps the dimmed fields behind it. Same pattern propagated to `bundle/TEMPLATE-editorial-v3.html` and `guidelines/update3-scene-illustrated.html`.
...
5. **Bundle template** — `bundle/TEMPLATE-editorial-v3.html` with every pattern as a copy-paste `<section>`.
6. **End-to-end example** — `ui_kits/ai-guide-non-tech/index.html`: 8-slide "6 istilah AI yang tiap developer wajib paham" carousel wiring the whole vocabulary together.
...
| `components/brand/` | `BrandMark`, `BrandHeader`. |
| `components/slide/` | `SlideShell`, `Eyebrow`, `PageCounter`, `InfoCard`, `NumberedStep`, `DarkCallout`, `Highlight`, `ImagePlate`, `ImagePlatePair`, `BigStat`, `PullQuote`, `SplitPanel`, `MediaGrid`. |
...
| `ui_kits/editorial-carousel/`, `ui_kits/react-problem-solving-carousel/`, `ui_kits/ai-guide-non-tech/` | Full editorial carousels using `deck-stage.js`. |
