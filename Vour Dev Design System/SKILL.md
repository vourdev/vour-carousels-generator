...
description: Use this skill to generate well-branded editorial carousels, slides, and adjacent assets for Vour Dev (@vourdev), either for production posts or throwaway prototypes/mocks. Contains the full DESIGN.md, pinned tokens, and a UI kit of reusable components.
user-invocable: true
---
...
- `components/` — reusable React components. Each has a `.prompt.md` next to it with a usage snippet.
- `guidelines/` — small specimen cards.
- `slides/` — standalone sample slides at 1080×1350.
- `ui_kits/` — full editorial carousels demonstrating the whole system end-to-end.
...
1. **Copy assets out.** If creating visual artifacts, copy `assets/vourdev-logo.jpeg` and any tokens/components/slides you use into the deliverable directory. Do not link across projects.
